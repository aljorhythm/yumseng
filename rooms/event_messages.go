...
func NewRoomConnectedMessage(room *Room, user User) (*RoomConnectedMessage, error) {
	...
	message := RoomConnectedMessage{RoomName: room.Name, UserId: user.GetId(), EventName: eventType.name}
	return &message, nil
}
...
func NewCheerAddedMessage(cheer cheers.Cheer) (*CheerAddedMessage, error) {
	...
	message := CheerAddedMessage{Cheer: cheer, EventName: eventType.name}
	return &message, nil
}
...
func NewRoomLastSecondsCheerCountMessage(count int) (*RoomLastSecondsCheerCountMessage, error) {
	...
	message := RoomLastSecondsCheerCountMessage{Count: count, EventName: eventType.name}
	return &message, nil
}
